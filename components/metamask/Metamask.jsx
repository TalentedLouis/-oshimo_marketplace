...
const Metamask_comp_text = () => {
  const { activateBrowserWallet, account, switchNetwork, library, deactivate } = useEthers()
  // const handleAfterConnect = async () => {
  //   await switchNetwork(Mainnet.chainId)
  //   // const signer = library.getSigner()
  //   // const signHash = await signer.signMessage("sign to accept tos")
...
const Metamask_comp_icon = ({ prop }) => {
  ...
  const handleAfterConnect = async () => {
    await switchNetwork(Mainnet.chainId)
    // 一旦コメントアウト　署名プロセス クッキーで署名管理実装する
    // const signer = library.getSigner()
    // const signHash = await signer.signMessage("sign to accept tos")
    // console.log(signHash);
  }
